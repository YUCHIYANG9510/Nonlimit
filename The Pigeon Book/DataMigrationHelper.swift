...
class DataMigrationHelper {
    ...
    /// 將舊的 2025 資料轉換為新格式
    /// 使用方式：
    /// 1. 將原本 calendarData 的所有資料複製到這裡
    /// 2. 執行這個函數
    /// 3. 將輸出結果複製到 LunarCalendarDataManager 的 load2025Data() 中
    static func convertOld2025DataToNewFormat() -> String {
        // ⚠️ 將你原本的 calendarData 字典貼在這裡
        let oldData: [String: (lunarTerm: String, suitableActivities: String, unsuitableActivities: String, idiom: String, idiomDescription: String, idiomImageName: String)] = [
        ...
        // 按日期排序
        let sortedKeys = oldData.keys.sorted()
        
        for (index, key) in sortedKeys.enumerated() {
            guard let data = oldData[key] else { continue }
            
            result += """
                "\(key)": LunarCalendarData(
                    lunarTerm: "\(data.lunarTerm)",
                    suitableActivities: "\(data.suitableActivities)",
                    unsuitableActivities: "\(data.unsuitableActivities)",
                    idiom: "\(data.idiom)",
                    idiomDescription: "\(data.idiomDescription)",
                    idiomImageName: "2025_\(data.idiomImageName)"
                ),
    ...
    /// 產生空白的年份資料模板
    /// 可以用這個快速產生 2026/2027/2028 的資料結構
    static func generateEmptyYearTemplate(year: Int) -> String {
        ...
        for day in 1...totalDays {
            ...
            result += """
                "\(dateKey)": LunarCalendarData(
                    lunarTerm: "待填入",
                    suitableActivities: "待填入",
                    unsuitableActivities: "待填入",
                    idiom: "待填入",
                    idiomDescription: "待填入",
                    idiomImageName: "\(year)_day\(String(format: "%03d", day))"
                ),
    ...
    /// 檢查資料完整性
    static func validateYearData(year: Int, data: [String: LunarCalendarData]) -> (isComplete: Bool, missingDates: [String]) {
        let calendar = Calendar.current
        let isLeapYear = (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0)
        let totalDays = isLeapYear ? 366 : 365
        ...
        for day in 1...totalDays {
            ...
            if data[dateKey] == nil {
                missingDates.append(dateKey)
            }
        }
        
        return (missingDates.isEmpty, missingDates)
    }
    
    /// 批次重命名圖片名稱
    /// 用於將 "day001" 改為 "2025_day001"
    static func generateImageRenameScript(year: Int, totalDays: Int = 365) -> String {
        var script = "#!/bin/bash\n\n"
        script += "# 批次重命名 \(year) 年圖片\n"
        script += "# 執行前請先備份圖片檔案！\n\n"
        
        for day in 1...totalDays {
            let oldName = "day\(String(format: "%03d", day))"
            let newName = "\(year)_day\(String(format: "%03d", day))"
            
            script += "mv \(oldName).png \(newName).png\n"
        }
    ...
    static func generateAssetsFolderStructure() -> String {
        ...
        ├── Images/
        │   ├── 2025/
        │   │   ├── 2025_day001.imageset/
        │   │   ├── 2025_day002.imageset/
        │   │   └── ... (365 個)
        │   ├── 2026/
        │   │   ├── 2026_day001.imageset/
        │   │   ├── 2026_day002.imageset/
        │   │   └── ... (365 個)
        │   ├── 2027/
        │   │   └── ...
        │   └── 2028/
...
extension DataMigrationHelper {
    /// 如何使用這個工具
    static func usage() {
        print("""
        
        ========================================
        DataMigrationHelper 使用說明
        ========================================
        
        1️⃣ 轉換 2025 年舊資料：
        ----------------------------------------
        let converted2025 = DataMigrationHelper.convertOld2025DataToNewFormat()
        print(converted2025)
        // 複製輸出結果到 load2025Data() 方法中
        
        
        2️⃣ 產生新年份空白模板：
        ----------------------------------------
        let template2026 = DataMigrationHelper.generateEmptyYearTemplate(year: 2026)
        print(template2026)
        // 複製後填入實際資料
        ...
        3️⃣ 檢查資料完整性：
        ----------------------------------------
        let yearData = LunarCalendarDataManager.shared.getAllAvailableDates(for: 2025)
        let validation = DataMigrationHelper.validateYearData(
            year: 2025,
            data: /* 你的資料字典 */
        )
        print("完整性: \\(validation.isComplete)")
        print("缺少的日期: \\(validation.missingDates)")
        
        
        4️⃣ 產生圖片重命名腳本：
        ----------------------------------------
        let script = DataMigrationHelper.generateImageRenameScript(year: 2025)
        print(script)
        // 將輸出儲存為 .sh 檔案並執行
        ...
        5️⃣ 查看建議的資料夾結構：
        ----------------------------------------
        let structure = DataMigrationHelper.generateAssetsFolderStructure()
        print(structure)
